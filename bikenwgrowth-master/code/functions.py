...
def simplify_ig_custom(G, attr):
    """Simplify an igraph with ox.simplify_graph
    """
    G_temp = copy.deepcopy(G)
    G_temp.es[attr] = G_temp.es["weight"]
    output = ig.Graph.from_networkx(ox.simplify_graph(nx.MultiDiGraph(G_temp.to_networkx())).to_undirected())
    output.es["weight"] = output.es[attr]
    return output
...
def ox_simplify_graph(G, attr, strict=True, remove_rings=True):
    """
